...
# Aggregate data by city and take weekly averages
df = df.groupby([pd.Grouper(level='City'),
                 pd.Grouper(level='date', freq='W')]).mean()
...
def apply_model(city_pd):
    ...
    print(city_pd.shape)
    train = city_pd.iloc[:-104]
    test = city_pd.iloc[-104:]
    ...
    start = len(train)
    end=(len(train)+len(test)-1) + 3*52
    pred = model.predict(start=start, end=end).rename('ARIMA Predictions')

    pred_series = pred
    newtest = test
    ix = pd.date_range(start=first_date, periods=260, freq='W')
...
results.write.format('bigquery') \
  .option('table', 'temp_predictions_dataset.sarimax_CityState_weekly') \
  .save()
